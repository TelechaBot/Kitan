...
async def handle_join_request(
    ...
    logger.info(
        ...
        f"--chat [{message.chat.title}]@{message.chat.username} --lang {message.from_user.language_code} --bio {message.bio} --name [{message.from_user.full_name}]"
    )
    # 尝试发送消息
    try:
        ...
        logger.error(f"join-request:send-welcome-failed:{chat_id}:{user_id}:{exc}")
    # 投入死亡队列
    await dead_shot(
        user_id=user_id,
        chat_id=chat_id,
    ...
    try:
        ...
        logger.error(f"join-request:save-history-failed:{chat_id}:{user_id}:{exc}")
    ...
    try:
        ...
        try:
            ...
            logger.error(f"join-request:send-verify-tip-failed:{chat_id}:{user_id}:{exc}")
    else:
